...
export class DatBanCommandService {
  ...
  async taoDatBan(authorization: string | undefined, body: BanGhi) {
    if (body.maKH) {
      await this.yeuCauKhachHangSoHuuTaiNguyen(
        authorization,
        String(body.maKH || ''),
      );
    }
    ...
    console.log('[DEBUG] taoDatBan body.chiTietMonAn:', JSON.stringify(body.chiTietMonAn));
    console.log('[DEBUG] taoDatBan body keys:', Object.keys(body));
    await this.mysql.thucThi(
      `INSERT INTO DatBan (MaDatBan, MaKH, MaBan, MaNV, NgayDat, GioDat, GioKetThuc, SoNguoi, GhiChu, TrangThai, KhuVucUuTien, ChiTietMonAn)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        body.maDatBan,
        body.maKH || null,
  ...
  async capNhatDatBan(
    ...
    return this.taoPhanHoi(
      this.datBanQueryService.chuyenDatBanSangResponse(datBanDaCapNhat),
      'Cap nhat dat ban thanh cong',
    );
  }

  async capNhatTrangThaiDatBan(
    authorization: string | undefined,
    maDatBan: string,
    trangThai: string,
  ) {
    ...
    await this.mysql.giaoDich(async (ketNoi) => {
      ...
      if (
        trangThaiDaChuanHoa === 'DA_XAC_NHAN' ||
        trangThaiDaChuanHoa === 'DA_GHI_NHAN' ||
        trangThaiDaChuanHoa === 'CAN_GOI_LAI' ||
        trangThaiDaChuanHoa === 'CHO_XAC_NHAN'
      ) {
        await ketNoi.execute('UPDATE Ban SET TrangThai = ? WHERE MaBan = ?', [
          'Reserved',
          maBan,
      ...
      if (
        trangThaiDaChuanHoa === 'DA_HOAN_THANH' ||
        trangThaiDaChuanHoa === 'KHONG_DEN' ||
        trangThaiDaChuanHoa === 'TU_CHOI_HET_CHO' ||
        trangThaiDaChuanHoa === 'DA_HUY'
      ) {
        await ketNoi.execute('UPDATE Ban SET TrangThai = ? WHERE MaBan = ?', [
          'Available',
          maBan,
    ...
    return this.taoPhanHoi(
      this.datBanQueryService.chuyenDatBanSangResponse(datBan),
      'Cap nhat dat ban thanh cong',
    );
  }

  async ganBanChoDatBan(
    authorization: string | undefined,
    maDatBan: string,
    body: BanGhi,
  ) {
    ...
    if (soKhach > 0 && tongSucChua < soKhach) {
      throw new BadRequestException(
        'Tong suc chua cua cac ban duoc chon khong du cho booking nay.',
      );
    }
    ...
    if (trangThaiKhongHopLe) {
      throw new BadRequestException(
        `Ban ${trangThaiKhongHopLe.MaBan} dang co khach, khong the gan cho booking.`,
      );
    }
