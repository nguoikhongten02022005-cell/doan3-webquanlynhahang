...
- script `database/mysql_init_quannhahang.sql` hiện đã gồm dữ liệu mẫu mở rộng cho các nhóm chức năng chính: tài khoản, bàn, đặt bàn, đơn hàng, mang về, đánh giá, điểm tích lũy, thông báo
...
Frontend chỉ gọi API thật khi bật backend mode. Cần cấu hình trong `frontend/.env` hoặc môi trường chạy:
...
Nếu thiếu một trong hai biến này, frontend sẽ fallback sang offline/mock ở nhiều API và bạn sẽ không nhìn thấy đầy đủ dữ liệu từ MySQL.
