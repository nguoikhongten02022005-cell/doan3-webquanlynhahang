...
const CREATE_STATUS_OPTIONS = INTERNAL_BOOKING_CREATE_STATUSES.filter((status) => Boolean(HOST_BOOKING_STATUS_LABELS[status]))
...
function BookingsTab({
  ...
  const visibleBookings = useMemo(
    () => bookingQueue.filter((booking) => matchesBookingSearch(booking, searchQuery)),
  ...
  const openAssignModal = (booking) => {
    const options = getAvailableTablesForBooking(booking)
  ...
  const handleConfirmAction = async () => {
    ...
    if (!result?.success) {
      setFormError(result?.error || 'Không thể thực hiện thao tác này.')
      return
    }
              ...
              <label className="form-group internal-dish-field" htmlFor="internal-booking-status">
                ...
                <select id="internal-booking-status" className="form-input" value={formValues.status} onChange={handleChange('status')}>
                  {CREATE_STATUS_OPTIONS.map((status) => (
                    <option key={status} value={status}>{HOST_BOOKING_STATUS_LABELS[status]}</option>
                  ))}
                </select>
              </label>
            ) : null}
      ...
      {visibleBookings.length === 0 ? (
        ...
        <div className="host-booking-list internal-list-top-gap">
          {visibleBookings.map((booking) => {
            ...
            const canAssignTables = canAssignBookingTables(booking)
            const canCheckIn = canCheckInBooking(booking)
            const canComplete = canCompleteBooking(booking)
            const canNoShow = canMarkBookingNoShow(booking)
        ...
        <div className="internal-booking-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="booking-assign-modal-title" onClick={resetAssignModal}>
          <div className="internal-booking-modal profile-card" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="host-booking-meta internal-booking-modal-meta">
              ...
              <p><span>Số khách</span><strong>{formatGuests(assigningBooking.guests)}</strong></p>
              <p><span>Khu vực ưu tiên</span><strong>{getSeatingLabel(assigningBooking.seatingArea)}</strong></p>
              <p><span>Thời gian</span><strong>{formatDateTime(assigningBooking.date, assigningBooking.time)}</strong></p>
            </div>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <span>{getSeatingLabel(table.areaId)} · {table.capacity} khách</span>
                    </div>
