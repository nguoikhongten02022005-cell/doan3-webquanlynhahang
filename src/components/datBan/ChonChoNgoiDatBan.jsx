...
function ChonChoNgoiDatBan({ formData, guestCount, handleSeatingSelect, selectedSeatOperationalNote }) {
  return (
        ...
        <>
          ...
          <div className="booking-seating-grid-premium">
            {CAC_KHU_VUC_DAT_BAN.map((area) => {
              const disabled = nenVoHieuHoaLuaChonChoNgoi(area.value, guestCount)
              const isSelected = formData.seatingArea === area.value
              const disabledHint = layGoiYVoHieuHoaChoNgoi(area.value, guestCount)
              ...
              return (
                <label key={area.value} className={`seating-area-option booking-seat-card ${isSelected ? 'selected' : ''} ${disabled ? 'disabled' : ''}`}>
                  ...
                  <span className="booking-seat-note">{layGhiChuVanHanhChoNgoi(area.value)}</span>
